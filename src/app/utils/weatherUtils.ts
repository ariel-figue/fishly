interface WeatherInfo {
  ...
  export const formatTimeTo12Hour = (timeString: string): string => {
    ...
    const [_, time] = timeString.split(" "); // Use _ to ignore date
    if (!time) return "N/A"; // Check if time exists
    const [hours, minutes] = time.split(":");
    if (!hours || minutes === undefined) return "N/A"; // Handle malformed time
    const hourNum = parseInt(hours, 10);
    if (Number.isNaN(hourNum)) return "N/A"; // Handle invalid hours
  ...
  export const calculateFishingScore = (
    ...
    let score = 100;
    const { wind_mph, cloud, temp, pressure_mb, precip_mm, uv, vis_miles } = weatherInfo;
    ...
    if (tides.length > 0) {
      ...
      const closestTide = tideTimes.reduce((prev: number, curr: number) =>
        Math.abs(curr - now.getTime()) < Math.abs(prev - now.getTime()) ? curr : prev
      );
